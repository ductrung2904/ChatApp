import React from 'react'

function MessageBoard() {
    return (
        <div className="chat__message__list">
            <div className="message__time">15:15, November 1</div>
            {/* <div className={styles["message__row"] + " " + styles["other__message"]}> */}
            <div className="message__row other__message">
                <div className="message__content">
                    <img src="https://scontent.xx.fbcdn.net/v/t1.6435-1/p100x100/237058448_2891263887792724_558775700647028364_n.jpg?_nc_cat=110&ccb=1-5&_nc_sid=dbb9e7&_nc_ohc=BaIn8vGmlpsAX80N9f8&_nc_ad=z-m&_nc_cid=0&_nc_ht=scontent.xx&oh=992e2399251b7531b909f24aa1933e51&oe=61A9F366" alt="" />
                    <div className="message__username">Dang Lai</div>
                    <div className="message__text">
                        thêm thằng vũ dô
                    </div>
                </div>
            </div>
            <div className="message__row you__message">
                <div className="message__content">
                    <div className="message__username">Đức Trung</div>
                    <div className="message__text">
                        Ok
                    </div>
                </div>
            </div>
            <div className="message__row other__message">
                <div className="message__content">
                    <img src="https://scontent.xx.fbcdn.net/v/t1.6435-1/p100x100/237058448_2891263887792724_558775700647028364_n.jpg?_nc_cat=110&ccb=1-5&_nc_sid=dbb9e7&_nc_ohc=BaIn8vGmlpsAX80N9f8&_nc_ad=z-m&_nc_cid=0&_nc_ht=scontent.xx&oh=992e2399251b7531b909f24aa1933e51&oe=61A9F366" alt="" />
                    <div className="message__username">Dang Lai</div>
                    <div className="message__text">
                        hello vũ trẻ trâu
                    </div>
                </div>
            </div>
            <div className="message__row other__message">
                <div className="message__content">
                    <img src="https://scontent.fsgn5-4.fna.fbcdn.net/v/t1.6435-1/c0.50.200.200a/p200x200/74165654_2403520756576492_4074453775123742720_n.jpg?_nc_cat=102&ccb=1-5&_nc_sid=7206a8&_nc_ohc=PNJ15Amcc0AAX-A1mzJ&_nc_ht=scontent.fsgn5-4.fna&oh=26dc007f68fa2bf0e8fee964105ba75f&oe=61AD05B1" alt="" />
                    <div className="message__username">Nguyễn Sơn Vũ</div>
                    <div className="message__text">
                        Tên riêng viết hoa vô
                    </div>
                </div>
            </div>
            <div className="message__row other__message">
                <div className="message__content">
                    <img src="https://scontent.xx.fbcdn.net/v/t1.6435-1/p100x100/237058448_2891263887792724_558775700647028364_n.jpg?_nc_cat=110&ccb=1-5&_nc_sid=dbb9e7&_nc_ohc=BaIn8vGmlpsAX80N9f8&_nc_ad=z-m&_nc_cid=0&_nc_ht=scontent.xx&oh=992e2399251b7531b909f24aa1933e51&oe=61A9F366" alt="" />
                    <div className="message__username">Dang Lai</div>
                    <div className="message__text">
                        ok nGuyễn sƠn vŨ
                    </div>
                </div>
            </div>
            <div className="message__row other__message">
                <div className="message__content">
                    <img src="https://scontent.xx.fbcdn.net/v/t1.6435-1/p100x100/237058448_2891263887792724_558775700647028364_n.jpg?_nc_cat=110&ccb=1-5&_nc_sid=dbb9e7&_nc_ohc=BaIn8vGmlpsAX80N9f8&_nc_ad=z-m&_nc_cid=0&_nc_ht=scontent.xx&oh=992e2399251b7531b909f24aa1933e51&oe=61A9F366" alt="" />
                    <div className="message__username">Dang Lai</div>
                    <div className="message__text">
                        chào mọi người, mình tên Lai, sinh viên khoa công nghệ thông tin khóa 18.
                        nhóm chúng ta có 8 thành viên nhưng chưa đủ nên các bạn có quen ai thì join vào nhóm này để tiện trao đổi.
                    </div>
                </div>
            </div>
            <div className="message__row other__message">
                <div className="message__content">
                    <img src="https://scontent.xx.fbcdn.net/v/t1.6435-1/p100x100/237058448_2891263887792724_558775700647028364_n.jpg?_nc_cat=110&ccb=1-5&_nc_sid=dbb9e7&_nc_ohc=BaIn8vGmlpsAX80N9f8&_nc_ad=z-m&_nc_cid=0&_nc_ht=scontent.xx&oh=992e2399251b7531b909f24aa1933e51&oe=61A9F366" alt="" />
                    <div className="message__username">Dang Lai</div>
                    <div className="message__text">
                        có ai muốn làm nhóm trưởng không??
                    </div>
                </div>
            </div>
            <div className="message__row other__message">
                <div className="message__content">
                    <img src="https://scontent.xx.fbcdn.net/v/t1.6435-1/p100x100/237058448_2891263887792724_558775700647028364_n.jpg?_nc_cat=110&ccb=1-5&_nc_sid=dbb9e7&_nc_ohc=BaIn8vGmlpsAX80N9f8&_nc_ad=z-m&_nc_cid=0&_nc_ht=scontent.xx&oh=992e2399251b7531b909f24aa1933e51&oe=61A9F366" alt="" />
                    <div className="message__username">Dang Lai</div>
                    <div className="message__text">
                        vì thấy nhóm 8 chưa ai tạo nên mình chỉ tạo cho nhanh thôi, nên ai muốn làm trưởng nhóm thì cứ mạnh dạn đề xuất nha.
                    </div>
                </div>
            </div>
            <div className="message__row other__message">
                <div className="message__content">
                    <img src="https://scontent.fsgn5-4.fna.fbcdn.net/v/t1.6435-1/c0.50.200.200a/p200x200/74165654_2403520756576492_4074453775123742720_n.jpg?_nc_cat=102&ccb=1-5&_nc_sid=7206a8&_nc_ohc=PNJ15Amcc0AAX-A1mzJ&_nc_ht=scontent.fsgn5-4.fna&oh=26dc007f68fa2bf0e8fee964105ba75f&oe=61AD05B1" alt="" />
                    <div className="message__username">Nguyễn Sơn Vũ</div>
                    <div className="message__text">
                        Của bạn hết đấy
                    </div>
                </div>
            </div>
            <div className="message__row other__message">
                <div className="message__content">
                    <img src="https://scontent.xx.fbcdn.net/v/t1.6435-1/p100x100/237058448_2891263887792724_558775700647028364_n.jpg?_nc_cat=110&ccb=1-5&_nc_sid=dbb9e7&_nc_ohc=BaIn8vGmlpsAX80N9f8&_nc_ad=z-m&_nc_cid=0&_nc_ht=scontent.xx&oh=992e2399251b7531b909f24aa1933e51&oe=61A9F366" alt="" />
                    <div className="message__username">Dang Lai</div>
                    <div className="message__text">
                        hi mn, vì ko ai đề xuất nên mình xin làm trưởng nhóm. vì chỉ học môn này chỉ học trong 6 tuần thôi nên mn cố gắng hoàn thành bài tập đúng hạn để kịp tổng hợp bài và nộp ha.
                    </div>
                </div>
            </div>
            <div className="message__row other__message">
                <div className="message__content">
                    <img src="https://scontent.xx.fbcdn.net/v/t1.6435-1/p100x100/237058448_2891263887792724_558775700647028364_n.jpg?_nc_cat=110&ccb=1-5&_nc_sid=dbb9e7&_nc_ohc=BaIn8vGmlpsAX80N9f8&_nc_ad=z-m&_nc_cid=0&_nc_ht=scontent.xx&oh=992e2399251b7531b909f24aa1933e51&oe=61A9F366" alt="" />
                    <div className="message__username">Dang Lai</div>
                    <div className="message__text">
                        văn mẫu
                    </div>
                </div>
            </div>
        </div>
    )
}

export default MessageBoard
